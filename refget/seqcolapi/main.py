"""The seqcolapi services: the PostgreSQL-backed ``app`` and the store-backed
``store_app``.

Importing this module builds a FastAPI app at module scope, and -- unless
``REFGET_STORE_URL`` or ``REFGET_STORE_PATH`` is set -- connects to PostgreSQL.
That is why :mod:`refget.seqcolapi` does not import it for you; ask for it by
name (``uvicorn refget.seqcolapi.main:app``).
"""
