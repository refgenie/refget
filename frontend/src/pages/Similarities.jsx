...
import { fetchSimilarities, fetchSimilaritiesJSON, fetchComparison, fetchComparisonJSON } from '../services/fetchData.jsx'
...
const Similarities = () => {
  ...
  const [selectedCollectionsIndex, setSelectedCollectionsIndex] = useState(collections.results.map(() => false));
  ...
  const handleNavigateSCIM = async (similiarityRow) => {
    try {
      ...
      if (similiarityRow.custom) {
        comparison = await fetchComparisonJSON(similiarityRow.raw, similiarityRow.comparedDigest);
      } else {
        comparison = await fetchComparison(similiarityRow.selectedDigest, similiarityRow.comparedDigest);
      }
  ...
  const handleRelationshipChange = (newRelationship) => {
    if (newRelationship !== relationship) {
      if (newRelationship === 'oneToMany' && relationship === 'manyToMany' && customCount > 1) {
    ...
    try {
      ...
      if (result?.similarities) {
        ...
        if (relationship === 'oneToMany') {
          setCustomCollections([{ selectedDigest: name !== '' ? name : customDigest, similarities: flattenedSimilarities }]);
          setSelectedCollectionsIndex(prev => [...prev.slice(0, collections.results.length), true]);
          setCustomCount(1);
        } else {
  ...
  useEffect(() => {
    const fetchAllSimilarities = async () => {
      ...
      for (let i = 0; i < selectedCollectionsIndex.length; i++) {
        ...
        if (i < collections.results.length && relationship === 'manyToMany') { // server collection
          try {
          ...
          <div className='d-flex align-items-end justify-content-between'>
            ...
            <ul className="nav nav-pills border border-2 border-light-subtle rounded rounded-3 bg-body-secondary">
              <li className="nav-item pointer-cursor">
                <span className={`nav-link px-3 py-1 me-1 tiny cursor-pointer fw-medium ${relationship === 'oneToMany' && 'active'}`} onClick={() => handleRelationshipChange('oneToMany')}>One-to-Many</span>
              </li>
              <li className="nav-item pointer-cursor">
                <span className={`nav-link px-3 py-1 me-1 tiny cursor-pointer fw-medium ${relationship === 'manyToMany' && 'active'}`} onClick={() => handleRelationshipChange('manyToMany')}>Many-to-Many</span>
              </li>
            </ul>
          </div>

          <p className='mt-2 mb-0 text-muted'>This tool provides similarity metrics for comparisons between all sequence collections on the server and one of your choice.</p>  
          <p className='mb-2 text-muted'>If you would like to view metrics for multiple sequence collections at once, use the "Many-to-Many" tab.</p>
        
          <div className='row mt-4'>

            <div className={`${relationship === 'manyToMany' ? 'col-6' : 'col-12'}`}>
              <div className='card'>
                <div className='card-header tiny d-flex justify-content-between'>
                  <span className='fw-bold'>Custom Collection Output</span>
                  <button className='btn btn-success btn-xs shadow-sm' onClick={async () => handleAddCustomCollection(customCollectionJSON, customCollectionName)}>{relationship === 'oneToMany' ? 'Compare' : 'Add'}</button>
                </div>
                <input 
          ...
          <div className='col-12'>
            <div className='d-flex align-items-end justify-content-between mt-4 mb-2'>
              ...
              <select className='form-select form-select-sm w-25' aria-label='strip-jitter' value={stripJitter} onChange={e => setStripJitter(e.target.value)}>
                ...
                {relationship === 'manyToMany' && <option value='uniform'>Uniformly Distributed Points</option>}
                {/* {relationship === 'manyToMany' && <option value='normal'>Normally Distributed Points</option>} */}
                {relationship === 'oneToMany' && <option value='bars'>Bars</option>}
              </select>
            </div>
              ...
              <table className='table table-striped table-hover table-rounded'>
                ...
                <tbody>
                  {similarities?.map((row, index) => (
                    <tr key={index} className='cursor-pointer' onClick={() => handleNavigateSCIM(row)}>
