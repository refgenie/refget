...
const HomePage = () => {
  ...
  return (
    <div className='home mb-5'>
      <div className='d-flex justify-content-end float-end ms-3'>
        <div className='card'>
          <div className='card-body tiny'>
            This server is hosting <b>{collections?.pagination?.total ?? 0}</b>{' '}
            collections.
          </div>
        </div>
      </div>
      ...
      <p>
        Welcome to the Refget Sequence Collections demo service! This landing
        page provides a way to explore the data in the server. You can go
        straight to the API itself using the <b>API Docs</b> link in the title
        bar. Or, you can check out a few examples below. Here are some ways to
        browse:
      </p>

      <h5>1. View and compare the demo sequence collections:</h5>
      <p className='text-muted fs-6'>
        This service includes several small demo collections. This page will
        show you comparisons between them:
      </p>

      <ul>
        <li>
          <Link to='/demo'>Demo of collection comparisons</Link>
        </li>
      </ul>
      ...
      <h5 className='mt-4'>
        3. Human Reference genomes available on this server:
      </h5>

      <ul>
        <li>
          <Link to='/human'>Human reference genomes</Link>
        </li>
      </ul>

      <h5 className='mt-4'>
        4. Human Pangenome Reference Consortium genomes available on this
        server:
      </h5>
      ...
      <p className='text-muted fs-6'>
        ...
        endpoint lets you retrieve a list of all the values of a particular
        attribute hosted by this server.
      </p>
      <AttributeList
        attributeName='name_length_pairs'
        attributeDigests={name_length_pairs}
      />
    </div>
  );
};
