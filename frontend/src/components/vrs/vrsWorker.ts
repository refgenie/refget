...
type RefgetStore = InstanceType<Gtars['RefgetStore']>;

/** One row of a collection's .rgsi sequence index. */
interface SeqMeta {
  name: string;
  length: number;
  alphabet: string;
...
/** Path templates read from a store's rgstore.json. */
interface StoreMeta {
  base: string;
  seqTemplate: string;
  collTemplate: string;
...
/** The one selected collection, plus its sequence index in both directions. */
interface ActiveGenome {
  base: string;
  seqTemplate: string;
  digest: string;
  label: string;
...
/** genomes.json: collection digest -> which store it came from and its sequences. */
...
/** stores.json: store base URL -> how it should be labelled in the cache panel. */
...
/** The wasm module after `initWasm`; every caller runs behind `wasmReady`. */
function wasm(): Gtars {
  if (!gtars) throw new Error('WASM module not initialised');
  return gtars;
}

/** The selected genome; every caller checks `genome` first. */
function activeGenome(): ActiveGenome {
  if (!genome) throw new Error('No genome selected. Pick a genome first.');
  return genome;
}

/** The wasm-side store the selected genome's chromosomes are loaded into. */
function activeStore(): RefgetStore {
  if (!residentStore) throw new Error('No genome selected. Pick a genome first.');
  return residentStore;
}

/** The digests already ingested into `residentStore`. */
function loadedDigests(): Set<string> {
  if (!loaded) throw new Error('No genome selected. Pick a genome first.');
  return loaded;
}
...
let wasmReady = false;
let storeMeta: StoreMeta | null = null; // after prepare-store
let genome: ActiveGenome | null = null; // after select-genome
let residentStore: RefgetStore | null = null; // wasm store; grows as chromosomes load
let loaded: Set<string> | null = null; // sha512t24u digests already ingested

function post(msg: unknown, transfer?: Transferable[]) {
  ctx.postMessage(msg, transfer || []);
}
function workerLog(text: string, error = false) {
...
ctx.addEventListener("message", async (ev: MessageEvent) => {
  ...
  try {
    ...
    post({ type: "error", text: err instanceof Error ? (err.stack ?? errorMessage(err)) : String(err) });
  }
});
...
// ===========================================================================
async function prepareStore(baseUrl: string) {
...
// ===========================================================================
async function selectGenome(digest: string, label: string) {
  ...
  residentStore = new (wasm().RefgetStore)();
  loaded = new Set();
...
async function selectCachedGenome(key: string) {
  const root = await navigator.storage.getDirectory();
  const dir = await root.getDirectoryHandle(OPFS_DIR, { create: true });
...
// collection index.
async function inspectGenome(key: string) {
...
function resolveChrom(name: string): SeqMeta | null {
  return activeGenome().byName.get(name) || null;
}
...
// already loaded).
async function ensureChromLoaded(dir: FileSystemDirectoryHandle, seqMeta: SeqMeta) {
  const g = activeGenome();
  const store = activeStore();
  const already = loadedDigests();
  if (already.has(seqMeta.sha512t24u)) return 0;
  const cacheName = `${seqMeta.sha512t24u}.seq`;
  const seqUrl = `${g.base}/${expandTemplate(g.seqTemplate, seqMeta.sha512t24u)}`;
  const { bytes, cached } = await ensureCached(dir, cacheName, seqUrl);

  if (isEncodedAlphabet(seqMeta.alphabet)) {
    store.add_encoded_sequence(
      seqMeta.name,
      seqMeta.sha512t24u,
      seqMeta.md5,
      seqMeta.length,
    ...
    store.add_sequence(seqMeta.name, bytes);
  }
  already.add(seqMeta.sha512t24u);
  workerLog(
...
// ===========================================================================
async function processVcf(buffer: ArrayBuffer, isGz: boolean, name: string) {
  if (!genome) {
  ...
  const count = wasm().vcf_to_vrs_ids(activeStore(), text, onResult);
...
async function processHgvs(lines: string[]) {
  if (!genome) {
    throw new Error("No genome selected. Pick a genome first.");
  }
  const root = await navigator.storage.getDirectory();
  ...
  for (const line of lines) {
    let row: string[];
    try {
      row = [line, store.hgvs_to_vrs_id(line)];
    } catch (e) {
      row = [line, `ERROR: ${e instanceof Error ? e.message : String(e)}`];
    }
    batch.push(row);
    processed++;
    if (batch.length >= RESULT_BATCH) {
...
function scanVcfChroms(text: string) {
...
function deriveLabel(base: string) {
  const parts = base.split("/").filter((p) => p && !p.endsWith(":"));
  return parts[parts.length - 1] || base;
}

async function readStores(dir: FileSystemDirectoryHandle): Promise<Stores> {
  try {
...
async function writeStores(dir: FileSystemDirectoryHandle, stores: Stores) {
...
async function readRegistry(dir: FileSystemDirectoryHandle): Promise<Registry> {
  try {
...
async function writeRegistry(dir: FileSystemDirectoryHandle, reg: Registry) {
...
async function deleteGenome(key: string) {
  const root = await navigator.storage.getDirectory();
...
// ===========================================================================
async function ensureCached(dir: FileSystemDirectoryHandle, name: string, url: string) {
...
async function tryReadOpfs(dir: FileSystemDirectoryHandle, name: string) {
  try {
...
async function writeOpfs(dir: FileSystemDirectoryHandle, name: string, bytes: Uint8Array) {
...
// ===========================================================================
function expandTemplate(template: string, digest: string) {
  return template
...
function parseRgsi(text: string): SeqMeta[] {
  const out: SeqMeta[] = [];
...
function isEncodedAlphabet(alphabet: string) {
