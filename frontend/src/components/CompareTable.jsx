...
const CompareTable = ({ seqColDict }) => {
  ...
  function buildCompareLinks(seqColDigests) {
    ...
    for (let i = 0; i < seqColDigests.length; i++) {
      header_cells.push(
        <th key={'header_col_' + i} className='rotated-text'>
          <div>{seqColNames[i]}</div>
        </th>,
      );
    }
    let header_row = (
    ...
    for (let i = 0; i < seqColDigests.length; i++) {
      let link_cells = [];
      link_cells.push(
        <th className='text-end' key={'header_row_' + i}>
          <Link to={`/collection/${seqColDigests[i]}`}>{seqColNames[i]}</Link>
        </th>,
      );
