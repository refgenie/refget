...
interface NetworkGraphProps {
  ...
  /** Which similarity attribute drives edge colour and the threshold. */
  metric?: string;
  /** 0 draws straight chords; higher values bow them toward the centre. */
  tension?: number;
  threshold?: number;
}

/** Where one digest sits on the ring, plus the label drawn for it. */
...
const NetworkGraph = ({
  ...
  // Debounces the resize redraw. A ref, not a window property: the graph owns
  // this timer and two mounted graphs must not share one.
  ...
  const drawNetwork = useCallback(() => {
    ...
    const allNodeSet = new Set<string>();
    similarities.forEach((d) => {
      if (d.selectedDigest && d.selectedDigest !== d.comparedDigest) {
    ...
    filteredSimilarities.forEach((d) => {
      if (d.selectedDigest) connectionCount[d.selectedDigest]++;
    ...
    container
      ...
      .attr('d', (d) => {
        const source = nodePositions[d.selectedDigest as string];
      ...
      .style('stroke', (d) => d3.interpolateBuPu(Number(d[metric] ?? 0)))
      .style(
        'stroke-width',
      ...
      .on('mouseover', function (event, d) {
        ...
        tooltip
          .html(
            `
          <strong>Digest 1:</strong> ${nodePositions[d.selectedDigest as string].id}<br/>
          <strong>Digest 2:</strong> ${nodePositions[d.comparedDigest].id}<br/>
          <strong>${snakeToTitle(metric)}:</strong> ${Number(d[metric] ?? 0).toFixed(3)}
        `,
          )
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      resizeTimer.current = setTimeout(drawNetwork, 150);
    };
  ...
  return (
    <div className='w-full'>
      <svg ref={svgRef} className='w-full network-svg' />
    </div>
  );
};
